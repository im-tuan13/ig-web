...
<div class="mb-3 flex items-end gap-2 {{ $mine ? 'justify-end' : 'justify-start' }}" data-message-id="{{ $message->id }}" data-message-created-at="{{ $message->created_at->toIso8601String() }}">
    @if (! $mine)
        ...
        @endif
    @endif
    <div @class(['max-w-[78%] rounded-[22px] px-4 py-2.5 text-sm leading-relaxed sm:max-w-[70%]', 'bg-[#0095f6] text-white' => $mine && $message->type !== 'sticker', 'border border-neutral-200 bg-white text-neutral-900' => ! $mine && $message->type !== 'sticker', 'bg-transparent px-1 py-0 text-5xl' => $message->type === 'sticker'])>
        ...
        @else
            @if ($message->post_id)
                @if ($message->post)
                    <a href="{{ route('posts.show', $message->post) }}" data-post-modal class="mb-1.5 block w-56 max-w-full overflow-hidden rounded-xl border {{ $mine ? 'border-white/30' : 'border-neutral-200' }}">
                        ...
                        @if ($message->post->caption)
                            <span class="block px-2.5 py-1.5 text-xs {{ $mine ? 'text-white/90' : 'text-neutral-700' }}">{{ \Illuminate\Support\Str::limit($message->post->caption, 60) }}</span>
                        @endif
                    </a>
                @else
                ...
                @endif
            @endif
            @if ($message->story_id)<p class="mb-1 text-xs italic">{{ $mine ? 'You replied to a story' : 'Replied to your story' }}</p>@endif
            @if ($message->message)<p>{{ $message->message }}</p>@endif
            <time class="message-time mt-0.5 block text-right text-[10px] opacity-70" datetime="{{ $message->created_at->toIso8601String() }}">{{ $message->created_at->format('g:i A') }}</time>
        @endif
    </div>
</div>
